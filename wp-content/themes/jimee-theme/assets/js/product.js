...
/* Auto-scroll labels infinite loop — mobile only */
document.addEventListener('DOMContentLoaded', function() {
    const labels = document.querySelector('.pd-labels');
    if (!labels || labels.children.length < 3) return;
    // scroll on all screens
    
    const items = Array.from(labels.children);
    items.forEach(item => {
        const clone = item.cloneNode(true);
        labels.appendChild(clone);
    });
    
    labels.classList.add('auto-scroll');
});
